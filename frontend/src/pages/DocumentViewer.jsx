...
const DocumentViewer = () => {
    ...
    useEffect(() => {
        const fetchDocumentUrl = async () => {
            try {
                const { data } = await axios.get(`http://localhost:5000/api/documents/${id}/url`, {
    ...
    const handleSummarize = async () => {
        ...
        setIsLoadingAI(true);
        try {
            const { data } = await axios.post('http://localhost:5000/api/ai/summarize', {
                documentId: id
            }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const handleExplain = async () => {
        ...
        setIsLoadingAI(true);
        try {
            const { data } = await axios.post('http://localhost:5000/api/ai/explain', {
                documentId: id,
                concept
            }, { headers: { Authorization: `Bearer ${token}` } });
